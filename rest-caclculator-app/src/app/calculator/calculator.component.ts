...
export class CalculatorComponent implements OnInit {
 ...
 setOperation(operation: string, symbol: string){
  ...
  reterieveHistory(){
       this.restCalculator.reteriveHistory().subscribe(
        res =>{
          if(!res.error){

            for(let i=0; i<res.history.length; i++){
    console.log(res.history[i].operand1 + res.history[i].operator + res.history[i].operand2 + "=" + res.history[i].result); //use i instead of 0
    this.simpleItems.push(res.history[i].operand1 + res.history[i].operator + res.history[i].operand2 + "=" + res.history[i].result);
}
